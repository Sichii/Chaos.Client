...
### 1. Unified asset format (replacing HPF/EPF/MPF/SPF/EFA)

**Current.** Legacy palettized formats in DAT archives. Dye system constrained to 6 palette slots. Palette cycling for water/lava/glow requires a dedicated subsystem (`PaletteCyclingManager`). Artist authoring is through HPF-aware tooling; no direct PNG/Aseprite workflow.
...
| **Dev total** | **6-10** |
| Artist migration (re-importing all assets) | Open-ended, artist-months |

**Strategic value.** Highest of any item in this doc. The assets produced survive the Godot port untouched — PNG + JSON are first-class in Godot. Every hour of artist migration work doubles as Godot port prep. Unblocks modern dye/shader experimentation. Supersedes the `PaletteCyclingManager` subsystem entirely.
...
| 1. Unified asset format (dev only; artist migration separate) | 6-10 |
...
| **Totals** | |
| Must-haves for bridge-client modernization (1-8 minus 3a) | ~34-48 |
| + auras/channels | ~39-56 |
...
**Two-engineer calendar estimate:** ~4-6 months elapsed for the must-haves, assuming one on client-dominant work and one on server-heavy tracks.
...
- Asset format (same ZIP containers, same manifest schema)
