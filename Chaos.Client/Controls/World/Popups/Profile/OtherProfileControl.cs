...
public sealed class OtherProfileControl : UIPanel
{
    ...
    private readonly TextElement ClassLabel;

    private readonly TextElement GroupStatusLabel;
    private readonly TextElement GuildLabel;
    private readonly TextElement LegendHeaderLabel;
    ...
    private readonly TextElement NameLabel;
    private readonly TextElement TitleLabel;
    ...
    private List<string>? ProfileTextLines;
    ...
    public OtherProfileControl()
    {
        ...
        ClassLabel = new TextElement();
        GuildLabel = new TextElement();
        TitleLabel = new TextElement();
        GroupStatusLabel = new TextElement();
        LegendHeaderLabel = new TextElement();
    ...
    private void ComputeLayout()
    {
        ...
        if (GuildLabel.HasContent)
            y += LINE_HEIGHT + 2;

        // Title line (only if present)
        if (TitleLabel.HasContent)
            y += LINE_HEIGHT + 2;
    ...
    public override void Draw(SpriteBatch spriteBatch)
    {
        ...
        // Profile text
        if (ProfileTextLines is not null)
        {
            ...
            TextRenderer.DrawLines(
                spriteBatch,
                new Vector2(ScreenX + PADDING, ScreenY + ProfileSectionY),
                ProfileTextLines,
                MAX_PROFILE_HEIGHT,
        ...
        string? profileText)
    {
        ...
        // Profile text — pre-wrap lines for draw-time rendering
        ProfileTextLines = null;
        ProfileTextHeight = 0;

        if (!string.IsNullOrWhiteSpace(profileText))
        {
            ProfileTextLines = TextRenderer.WrapText(profileText, PROFILE_TEXT_WIDTH);
            var totalHeight = ProfileTextLines.Count * TextRenderer.CHAR_HEIGHT;
            ProfileTextHeight = Math.Min(totalHeight, MAX_PROFILE_HEIGHT);
        }
