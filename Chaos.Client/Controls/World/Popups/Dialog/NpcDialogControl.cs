...
public sealed class NpcDialogControl : PrefabPanel
{
    ...
    public override void Draw(SpriteBatch spriteBatch)
    {
        ...
        for (var i = 0; i < Options.Count; i++)
        {
            ...
            opt.Body.Update(opt.Text, color);
            opt.Body.Draw(spriteBatch, new Vector2(sx + opt.Rect.X, sy + opt.Rect.Y));
        }
    }
    ...
    {
        if (TextLabel is null || (TextRect == Rectangle.Empty))
            return;
        ...
        {
            TextLabel.ForegroundColor = Color.White;
            TextLabel.Text = text;
