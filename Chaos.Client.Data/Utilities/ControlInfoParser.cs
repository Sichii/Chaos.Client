...
public sealed class ControlInfoParser
{
    ...
    public UserControlDetails Parse(string name, Stream stream)
    {
        using var reader = new StreamReader(stream, Encoding.Default, true);
        var userControlInfo = new UserControlDetails(name);
        ControlDetails? currentControl = null;
        ...
        while (reader.ReadLine() is { } line)
        {
            ...
            if (string.IsNullOrEmpty(value))
            {
                switch (token)
                {
                    ...
                    {
                        currentControl = new ControlDetails();
